...
class EnhancedXLSXParser:
    ...
    def _detect_visually_merged_cells(self, sheet_name: str) -> None:
        ...
        for col in range(1, max_col + 1):
            ...
            column_header = col_info.get('header')
            ...
            for row in range(1, max_row + 1):
                ...
                if (row, col) in self.merged_cells_map.get(sheet_name, {}):
                    # Reset tracking if we hit an officially merged cell
                    current_value = None
                    start_row = None
                    continue
                ...
                if cell_value is None:
                    ...
                    # we might want to create a visual merge
                        ...
                        # Create a virtual merged range
                        range_coord = f"{get_column_letter(col)}{start_row}:{get_column_letter(col)}{row-1}"
                        master_cell_coord = (start_row, col)
                        master_cell = sheet.cell(row=start_row, column=col)
                        
                        # Only create visual merges for likely label/category columns
                        ...
                        # For safety, only map label columns or columns explicitly allowed to merge
                        if is_label_column:
            ...
            if start_row is not None and max_row - start_row > 0:
                ...
                range_coord = f"{get_column_letter(col)}{start_row}:{get_column_letter(col)}{max_row}"
                master_cell_coord = (start_row, col)
                master_cell = sheet.cell(row=start_row, column=col)
        ...
        # Process each row (horizontal merges) - similar logic
        for row in range(1, max_row + 1):
            current_value = None
            start_col = None
            ...
            for col in range(1, max_col + 1):
                ...
                if (row, col) in self.merged_cells_map.get(sheet_name, {}):
                    ...
                    if start_col is not None and col - start_col > 1:
                        # Create a virtual merged range
                        range_coord = f"{get_column_letter(start_col)}{row}:{get_column_letter(col-1)}{row}"
                ...
                # Skip empty cells with similar logic to vertical merges
                if cell_value is None:
                    # Process similar to vertical merges above...
                    if start_col is not None and col - start_col > 1:
                        # Create horizontal visual merge similar to above...
                        range_coord = f"{get_column_letter(start_col)}{row}:{get_column_letter(col-1)}{row}"
                        master_cell_coord = (row, start_col)
                        master_cell = sheet.cell(row=row, column=start_col)
                        ...
                        if should_create_merge:
                            # Store and map this horizontal merge similar to above...
                            self.merged_cells_info[sheet_name][range_coord] = {
                                'min_row': row,
                                'max_row': row,
                                'min_col': start_col,
                ...
                # Handle non-empty cells - similar to vertical merge logic
                # If this is a new value or the first non-empty cell
                if current_value != cell_value:
                    # Similar processing for horizontal merges...
                    if start_col is not None and col - start_col > 1:
                        # Process similar to above...
                        # Create horizontal visual merge
                        range_coord = f"{get_column_letter(start_col)}{row}:{get_column_letter(col-1)}{row}"
                        master_cell_coord = (row, start_col)
                        master_cell = sheet.cell(row=row, column=start_col)
            ...
            if start_col is not None and max_col - start_col > 0:
                # Similar to the last horizontal merge processing above...
                range_coord = f"{get_column_letter(start_col)}{row}:{get_column_letter(max_col)}{row}"
                master_cell_coord = (row, start_col)
                master_cell = sheet.cell(row=row, column=start_col)
    ...
    def _get_cell_style_summary(self, cell: Cell) -> Dict[str, Any]:
        """
        Get a summary of cell's style attributes.
        ...
        return {
            ...
            'fill': {
                'type': getattr(getattr(cell, 'fill', None), 'fill_type', None),
                'start_color': getattr(getattr(cell, 'fill', None), 'start_color', None),
            }
        }
    ...
    def _get_cell_value_and_metadata(self, sheet, row: int, col: int) -> Dict[str, Any]:
        """
        ...
        if (row, col) in self.merged_cells_map.get(sheet_name, {}):
            is_merged = True
            master_cell_coord = self.merged_cells_map[sheet_name][(row, col)]
        ...
        if (row, col) in self.cell_styles.get(sheet_name, {}):
            cell_style = self.cell_styles[sheet_name][(row, col)]
        else:
            cell_style = self._get_cell_style_summary(cell)
        ...
        result = {
            'value': value,
            'is_merged': is_merged,
            'is_visual_merge': is_visual_merge,
            'style': cell_style
        }
